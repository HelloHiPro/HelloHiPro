...
local function clicktpFunc()
	pcall(function()
		...
		local offset = 10 -- Distance in front
        local frontVector = rootPart.CFrame.LookVector * offset
        rootPart.CFrame = rootPart.CFrame + frontVector
	end)
end
...
addcmd('loopgoto',{},function(args, speaker)
	...
	for i,v in pairs(players)do
		...
		repeat
			if Players:FindFirstChild(v) then
				if Players[v].Character ~= nil then
                    local offset = distance -- Distance behind (negative for "behind")
                    local targetCFrame = getRoot(Players[v].Character).CFrame
                    local behindVector = targetCFrame.LookVector * offset
                    getRoot(speaker.Character).CFrame = targetCFrame + behindVector
				end
				wait(lDelay)
			else
				loopgoto = nil
